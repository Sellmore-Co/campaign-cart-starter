...
const CONFIG = {
  ...
  defaults: {
    ...
    // size: undefined, // No default size - user must select
  },
...
class TierController {
  ...
  _copySelectionsToNewSlots(fromTier, toTier) {
    ...
    for (let i = fromTier + 1; i <= toTier; i++) {
      // Only copy the color, let user select size for each slot
      this.selectedVariants.set(i, { color: slot1.color });
      this._updateSlot(i, { color: slot1.color });
      this._updateSlotPrice(i);
    }
  }
